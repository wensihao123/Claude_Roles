...
- <next> + <slug> MUST match the HANDOFF "下一步" you just wrote — never let them drift.
  (After ASSET-SPEC it's /image-prompt; after all assets pass it's the next stage or
  feature completion; if assets failed, the human redoes them, then re-accept.)
