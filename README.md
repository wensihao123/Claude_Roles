...
**开场先握手,确认再干活**:用 `/role-xxx <slug>`(或某个 skill 命令)激活后,role/skill
**不会**直接闷头产出/改 artifact,而是先用中文 1-3 行汇报"这是哪个功能、本 session 准备干什么",
再问一句「有什么要先补充或调整的吗?」,**等你确认后才开始**。这样你激活时想先交代两句,不必等它
跑完一轮才插得上话。握手只在 session 开头一次(不是每步都打断);纯查状态的提问会直接回答。
